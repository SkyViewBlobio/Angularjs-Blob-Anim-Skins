document.addEventListener("DOMContentLoaded", function () {
    var canvas = document.getElementById("shaderEightCanvas");
    ...
    var fsSource = `#extension GL_OES_standard_derivatives : enable
...
const vec3 lightPos1 = 100.0*vec3(-1.3, 1.9, 1.0);
...
float psy_warp(vec2 p, out vec2 v, out vec2 w) {
    ...
    vec2 pp = p;  // Create a copy of p to avoid modifying the original variable
    pp   -= eyeRef * pmax(0.0, dot(p, eyeRef), 0.25) * 2.0;
    pp   -= 0.25 * eyeRef;

    f = smoothstep(-0.1, 0.15, id);
    const float rep = 50.0;
    const float sm = 0.125 * 0.5 * 60.0 / rep;
    float  n = smoothKaleidoscope(pp, sm, rep);
    pp.y += TIME * 0.125 + 1.5 * g_psy_th;
    ...
    v = vec2(psy_fbm(pp + vx, aa), psy_fbm(pp + vy, aa)) * f;
    w = vec2(psy_fbm(pp + 3.0 * v + wx, aa), psy_fbm(pp + 3.0 * v + wy, aa)) * f;

    return -tanh_approx(psy_fbm(pp + 2.25 * w, aa) * f);
}
...
vec3 color(vec2 p) {
    ...
    vec3 ro = vec3(0.0, 10.0, 0.0);  // You may need to adjust this based on your scene
    vec3 pp = vec3(p.x, 0.0, p.y);

    // Keep the camera direction fixed
    vec3 po = vec3(p.x, 0.0, p.y);
    vec3 rd = normalize(po - ro);
...
void main() {
  ...
  // Apply circular mask
  float mask = smoothstep(1.0, 0.99, length(p));
  if (mask < 0.1) {
  ...
  gl_FragColor = vec4(col, 1.0);
    ...
    // Create positions for ShaderEight to represent a square
